...
__global__ void tensor_cores(int A_num_fil, int A_num_col,float *A, int B_num_fil, int B_num_col,float *B, float *C){
    ...
    for (int i = 0; i < A_num_col/tensor_K; i ++){
        ...
        for(int i = 0; i < 8; i ++){
            int thread_A_pointer = A_pointer + threadIdx.x%16 + (threadIdx.y%2)*A_num_col + i*A_num_col*2;
            int thread_B_pointer = B_pointer + threadIdx.x%16 + (threadIdx.y%2)*B_num_col + i*B_num_col*2;

            shared_memory_1[threadIdx.y%2 + i*2][threadIdx.x] = A[thread_A_pointer];
            shared_memory_2[threadIdx.y%2 + i*2][threadIdx.x] = B[thread_B_pointer];
        }
