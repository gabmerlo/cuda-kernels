%%writefile blocktiling-2d-float4rb.cu
#include <cstdio>
#include <random>
#include <chrono>
#include <algorithm>
...
__global__ void blocktiling_2d_float4(int A_num_fil, int A_num_col,float *A, int B_num_fil, int B_num_col,float *B, float *C){
...
//I implemented the same kernel but with float4 when loading into registers from b
__global__ void blocktiling_2d_float4registerb(int A_num_fil, int A_num_col,float *A, int B_num_fil, int B_num_col,float *B, float *C){
    ...
    __syncthreads();
        ...
        for (int i = 0; i < BK; i ++){
            ...
            regB[0] = f4_registerb.x;
            regB[1] = f4_registerb.y;
            regB[2] = f4_registerb.z;
            regB[3] = f4_registerb.w;
...
//Prepared this functions so that I can compare both versions, and later the transposed version too
...
int main(){
    ...
    //Calentamiento
    for (int i = 0; i < 3; i++){
        blocktiling_2d_float4<<<blocks,threads>>>(
            A_num_fil, A_num_col, d_A,
            B_num_fil, B_num_col, d_B, d_C
        );
        blocktiling_2d_float4registerb<<<blocks,threads>>>(
            A_num_fil, A_num_col, d_A,
            B_num_fil, B_num_col, d_B, d_C
        );
    ...
    for (int i = 0; i < N_ITER; i++) {
        ...
        blocktiling_2d_float4<<<blocks,threads>>>(A_num_fil, A_num_col, d_A, B_num_fil, B_num_col, d_B, d_C);
        cudaEventRecord(stop);
        cudaEventSynchronize(stop);
        cudaEventElapsedTime(&times[i], start, stop);

        cudaEventRecord(start);
        blocktiling_2d_float4registerb<<<blocks,threads>>>(A_num_fil, A_num_col, d_A, B_num_fil, B_num_col, d_B, d_C);
        cudaEventRecord(stop);
        cudaEventSynchronize(stop);
    ...
    report("Float4 sin rb", times, N_ITER, flops);
    report("Float4 en rb", times_2, N_ITER, flops);
